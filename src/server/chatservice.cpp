  ...
  if (user.getId() == id && user.getPassword() == pwd) {
    ...
    if (user.getState() == "online") {
      ...
      // 登录成功，记录用户信息，stl标准库没有考虑线程安全问题,使用mutex对map加锁
      {
        ...
        _userConnMap.insert(
            std::pair<int, muduo::net::TcpConnectionPtr>(user.getId(), conn));
      }
...
void ChatService::regiseter(const muduo::net::TcpConnectionPtr &conn, json &js,
                            muduo::Timestamp time) {
  ChaoticStreamCipher cipher{0.3, 0.4};
...
void ChatService::handleRedisSubscribeMessage(int userid,
  ...
  auto it = _userConnMap.find(userid);
  if (it != _userConnMap.end()) {
    std::string msg_ = cipher.encrypt(msg);
    int len = msg_.length();
    char buffer[4];
    memcpy(buffer, &len, sizeof(len));
