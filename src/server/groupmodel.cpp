...
{
    char sql[1024] = {0};
    sprintf(sql, "insert into groupuser valses(%d,%d,'%s')", groupid, userid, role.c_str());
    MySQL mysql;
    if (mysql.connect())
    {
        mysql.update(sql);
...
std::vector<Group> GroupModel::queryGroups(int userid)
{
    char sql[1024] = {0};
    sprintf(sql,
            "select a.id,a.groupname,a.groupdesc form allgroup a inner join groupuser b on a.id = b.groupid where b.userid=%d",
            userid);
